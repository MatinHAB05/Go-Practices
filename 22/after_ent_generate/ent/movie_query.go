...
// QueryCasts chains the current query on the "casts" edge.
func (_q *MovieQuery) QueryCasts() *CastQuery {
	query := (&CastClient{config: _q.config}).Query()
...
// WithCasts tells the query-builder to eager-load the nodes that are connected to
// the "casts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *MovieQuery) WithCasts(opts ...func(*CastQuery)) *MovieQuery {
	query := (&CastClient{config: _q.config}).Query()
...
func (_q *MovieQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Movie, error) {
	...
	if query := _q.withCasts; query != nil {
		if err := _q.loadCasts(ctx, query, nodes,
			func(n *Movie) { n.Edges.Casts = []*Cast{} },
			func(n *Movie, e *Cast) { n.Edges.Casts = append(n.Edges.Casts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MovieQuery) loadCasts(ctx context.Context, query *CastQuery, nodes []*Movie, init func(*Movie), assign func(*Movie, *Cast)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(movie.CastsTable)
		s.Join(joinT).On(s.C(cast.FieldID), joinT.C(movie.CastsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(movie.CastsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(movie.CastsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Cast](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "casts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
