...
func (s *Service) GetAllCastsOfMovie(movie_id int) ([]serviceparam.GetAllCastsOfMovieResponse, error) {
	casts, err := s.moviecastRepo.GetAllCastsOfMovie(movie_id)
	if err != nil {
		return nil, fmt.Errorf("get all casts of movie by movie_id id-%d : %w\n", movie_id, err)
	}
	final := make([]serviceparam.GetAllCastsOfMovieResponse, 0)
	for i := range casts {
		final = append(final, serviceparam.GetAllCastsOfMovieResponse{
			Id:     final[i].Id,
...
func (s *Service) GetAllMoviesOfCast(cast_id int) ([]serviceparam.GetAllMoviesOfCastResponse, error) {
	movies, err := s.moviecastRepo.GetAllMoviesOfCast(cast_id)
	if err != nil {
		return nil, fmt.Errorf("get all movies of cast by cast_id id-%d : %w\n", cast_id, err)
	}
	final := make([]serviceparam.GetAllMoviesOfCastResponse, 0)
	for i := range movies {
		final = append(final, serviceparam.GetAllMoviesOfCastResponse{
			Id:      final[i].Id,
